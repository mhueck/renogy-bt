...
class BaseClient:
    ...
    async def connect(self):
        ...
        if not self.ble_manager.device:
            ...
            if self.ble_manager.client and self.ble_manager.client.is_connected: await self.fetch_next()
    ...
    async def on_data_received(self, response):
        ...
        frame_header = bytes_to_int(response, 0, 1)
        operation = bytes_to_int(response, 1, 1)
        status = bytes_to_int(response, 2, 1)
        data_length = bytes_to_int(response, 3, 1)
        payload = response[4:]

        if frame_header == FRAME_HEADER and status == STATUS_READ_SUCCESS:
            ...
            logging.warning(f"unexpected frame header ({frame_header}) or wrong status ({status}).")
